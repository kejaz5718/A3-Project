...
public class BinaryTree
{
     ...
     * side)
     *
     * @param nod  	The node to start the reverse order walk from
     * @param depth	The current depth of the node in the tree
     */
    private void printTree(Node nod, int depth)
    {
        if (nod.hasRight())
            printTree(nod.right(), depth + 1);

        // Show depth of current code by indenting to the right
        for (int d = 0; d < depth; d++)
            System.out.print("  ");

        System.out.println(nod.getNodeObjectName());

        if (nod.hasLeft())
            printTree(nod.left(), depth + 1);
    }
     ...
     * @param stringb The StringBuffer
     * @param type The type of walk : "preOrder", "inOrder", "postOrder"
     */
    protected void orderWalking(Node currentn, StringBuffer stringb, String type)
    {
        if (currentn == null)
        {
            return;
        ...
        if ("inOrder".equalsIgnoreCase(type))
        {
            ...
            if (currentn.hasLeft())
            {
                orderWalking(currentn.left(), stringb, type);
            }
            //visit node
            stringb.append(currentn.getNodeObject().getFullName());
            stringb.append(", ");
            //recurse right
            if (currentn.hasRight())
            {
                orderWalking(currentn.right(), stringb, type);
            }
        }
    }
     ...
     */
    protected void ownWalking(Node currentNode, StringBuffer sbuffer)
    {
        if (currentNode == null)
        {
            return;
        ...
        {
            ownWalking(currentNode.right(), sbuffer);
        }
        // recurse left
        if (currentNode.hasLeft())
        {
            ownWalking(currentNode.left(), sbuffer);
        }
        // appends the last name
        sbuffer.append(currentNode.getNodeObject().getFullName());
        sbuffer.append(", ");
