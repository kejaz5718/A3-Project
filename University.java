...
public class University
{
     ...
     *
     * @param ind The index of the cohort that you want to call the method for
     ...
     */
    public String walkTree(int ind, String type)
    {
        return cohorts.get(ind).walkTree(type);
    }
    ...
    {
        //TODO done
        Boolean visiting = beingVisited.get(node);
        //to check if key is present and its value is true
        if (visiting != null && visiting)
        {
            return true;
        }
        Boolean BeenVisited = visited.get(node);
        //to check if key is present and its value is true
        if (BeenVisited != null && BeenVisited)
        {
            return false;
        }
    ...
    {
        ArrayList<Cohort> sortedList = this.sortMethod(true, "name"); // sorting before searching
        return searchForModule(sortedList, moduleName);
    }
    ...
    protected Module searchForModule(ArrayList<Cohort> list, String moduleName)
    {
        ...
        {
            int mid = b + (u - b) / 2;
            String current_moduleName = list.get(mid).getModule().getName();
            int currentmp = current_moduleName.compareTo(moduleName);
            if (currentmp == 0)
            {
    ...
    protected ArrayList<Cohort> sortMethod(ArrayList<Cohort> list, int block_size, boolean ascending, String attr) {
        ...
        for (int k = 1; k < a; k++)
        {
            Cohort key = list.get(k);
            int l = k - 1;

            while (l >= 0)
            {
                Cohort current = list.get(l);
                int difference;

                if (attr.equalsIgnoreCase("name"))
                {
                    difference = current.getModule().getName().compareTo(key.getModule().getName());
                }
                else
                {
                    difference = current.getModule().getCode() - key.getModule().getCode();
                }
